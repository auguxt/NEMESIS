...
def dblog(session):
    ...
    with _lock, sqlite3.connect(DB_PATH) as con:
        con.execute(
            ...
            (time.time(), session["service"], session["src_ip"], session["src_port"],
             session["session_id"], json.dumps(session["commands"]),
             json.dumps(session["credentials"]), session.get("drops", "")))
...
def log_honeytoken_hit(src_ip, label="decoy credential"):
    ...
    init_db()
    token = "tok_" + str(abs(hash(label)) % 10**9)
    with _lock, sqlite3.connect(DB_PATH) as con:
        con.execute("INSERT OR IGNORE INTO honeytokens (token, label, created) VALUES (?,?,?)",
                    (token, label, time.time()))
        row = con.execute("SELECT id FROM honeytokens WHERE token=?", (token,)).fetchone()
        con.execute("INSERT INTO honeytoken_hits (token_id, src_ip, timestamp) VALUES (?,?,?)",
